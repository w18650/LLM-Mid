...
def load_iwslt_data(data_path, max_samples=10000):
    ...
    for root, dirs, files in os.walk(extract_path):
        if 'train.tags.en-de.en' in files and 'train.tags.en-de.de' in files:
            train_dir = root
            break
    
    if train_dir is None:
        raise FileNotFoundError("Could not find train.tags.en-de.en and train.tags.en-de.de files")
    
    print(f"Found training data in: {train_dir}")
    
    src_file = os.path.join(train_dir, "train.tags.en-de.en")
    tgt_file = os.path.join(train_dir, "train.tags.en-de.de")
