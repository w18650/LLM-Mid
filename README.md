...
本仓库包含一个用于在 IWSLT2017 数据集（英语到德语）上进行机器翻译的 Transformer 模型的实现。它包括用于运行各种消融实验的代码，包括对大模型和小模型变体的比较。
...
此脚本将：
1.  训练大 Transformer 模型（默认参数：`d_model=512`, `nhead=8`, `num_encoder_layers=6`, `num_decoder_layers=6`, `dim_feedforward=2048`）。
2.  训练小 Transformer 模型（参数：`d_model=256`, `nhead=4`, `num_encoder_layers=2`, `num_decoder_layers=2`, `dim_feedforward=512`）。
3.  在 `./results/` 目录下生成一个对比图（`size_ablation_loss_comparison.png`）。
